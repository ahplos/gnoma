...
{{/*
  ======================================
  gnoma-kubernetes.configMap
  ======================================

  PARAMETERS LIST:
    . -> should always be root of chart
    $cmValues -> Gnoma template for ConfigMap
  ...
  Defines a Gnoma template for a Kubernetes ConfigMap.
*/}}
{{- define "gnoma-kubernetes.configMap" }}
  {{- $ := get . "$" }}
  {{- $cmValues := .gnomaTemplate }}
  ...
  {{- $whiteList := list "binaryData"
                         "data"
                         "immutable"	}}
  {{- include "gnoma-common.outputToYaml" (dict "$" $ "gnomaTemplate" $cmValues "whiteList" $whiteList "indent" 0) }}
{{- end }}

{{/*
  ======================================
  gnoma-kubernetes.secret
  ======================================

  PARAMETERS LIST:
    . -> should always be root of chart
    $secretValues -> Gnoma template for Secret
  ...
  Defines a Gnoma template for a Kubernetes Secret.
*/}}
{{- define "gnoma-kubernetes.secret" }}
  {{- $ := get . "$" }}
  {{- $secretValues := .gnomaTemplate }}
  ...
  {{- $whiteList := list "data"
                         ...
                         "stringData"
                         "type"	}}
  {{- include "gnoma-common.outputToYaml" (dict "$" $ "gnomaTemplate" $secretValues "whiteList" $whiteList "indent" 0) }}
{{- end }}

{{/*
  ======================================
  gnoma-kubernetes.persistentVolume
  ======================================

  PARAMETERS LIST:
    . -> should always be root of chart
    $pvValues -> Gnoma template for PersistentVolume
  ...
  Defines a Gnoma template for a Kubernetes PersistentVolume.
*/}}
{{- define "gnoma-kubernetes.persistentVolume" }}
  {{- $ := get . "$" }}
  {{- $pvValues := .gnomaTemplate }}
...
spec:
  {{- $whiteList := list "awsElasticBlockStore"
                         ...
                         "vsphereVolume"
                         "volumeMode"	}}
  {{- include "gnoma-common.outputToYaml" (dict "$" $ "gnomaTemplate" $pvValues "whiteList" $whiteList) }}
  accessModes:
...
{{/*
  ======================================
  gnoma-kubernetes.persistentVolumeClaim
  ======================================

  PARAMETERS LIST:
    . -> should always be root of chart
    $pvcValues -> Gnoma template for PersistentVolumeClaim
  ...
  Defines a Gnoma template for a Kubernetes PersistentVolumeClaim.
*/}}
{{- define "gnoma-kubernetes.persistentVolumeClaim" }}
  {{- $ := get . "$" }}
  {{- $pvcValues := .gnomaTemplate }}
...
spec:
  {{- $whiteList := list "dataSource"
                         ...
                         "volumeMode"
                         "volumeName"	}}
  {{- include "gnoma-common.outputToYaml" (dict "$" $ "gnomaTemplate" $pvcValues "whiteList" $whiteList) }}
  accessModes:
