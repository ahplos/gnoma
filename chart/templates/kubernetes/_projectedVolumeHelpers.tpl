...
{{- define "gnoma-kubernetes.projectedVolumes" }}
  {{- $ := get . "$" }}
  {{- $podValues := .podValues }}
  ...
  {{- range $projectedVolume := $containerVals.projectedVolumes }}
    ...
    {{- include "gnoma-kubernetes.addConfigMapsAndSecretsByLabels" $projectedVolumeArgs }}
    {{- include "gnoma-kubernetes.createProjectedVolume" $projectedVolumeArgs }}
...
{{- define "gnoma-kubernetes.createProjectedVolume" }}
  {{- $ := get . "$" }}
  {{- $podValues := .podValues }}
...
{{- define "gnoma-kubernetes.addConfigMapsAndSecretsByLabels" }}
  {{- $ := get . "$" }}
  {{- $podValues := .podValues }}
  {{- $projectedVolume := .projectedVolume }}
  ...
  {{- if or $projectedVolume.configMapsByLabels $projectedVolume.labels }}
    ...
    {{- include  "gnoma-kubernetes.getResourcesByLabel" (list $ $resources $resourceLabels $resultKey) }}
...
{{- define "gnoma-kubernetes.getResourcesByLabel" }}
  {{- $ = index . 0 }}
  {{- $resources := index . 1 }}
