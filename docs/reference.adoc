= Gnoma Chart Reference Documentation
...
NOTE: For the remainder of this document and unless otherwise explicitly noted, "templates" will refer only to Gnoma Chart templates, and **NOT** Helm templates.
...
Gnoma Chart is a 100% compatible Helm chart that does away with the need for Go templates and replaces it with a much simpler and -- in some cases -- more flexible alternative for defining Kubernetes deployment manifests.  No other tool is required other than Helm, and Gnoma Chart only uses strict YAML for defining it's deployment definitions.  The goal is to make sure there is no need to ever define a Helm/Go template again, and make creating Helm compatible templates easier much easier.

With Gnoma Chart values.yaml files have the following features:

Variables::

Define Gnoma Chart variables, which can hold any valid YAML type.
...
Gnoma Chart as a project is made up of two charts, a https://helm.sh/docs/topics/library_charts/[Helm library chart] and an application chart:

.Provided gnoma-charts with usage example
* **gnoma-lib**: library chart
+
...
- name: gnoma-lib
  version: 0.1.0
  repository: https://ahplos.github.io/Gnoma-deploy/charts/
----

* **gnoma-chart**: application chart
+
[source,YAML,linenums]
----
...
`helm registry login --username ${USERNAME} oci://ghcr.io/gnoma`

== Deployment Definitions

In Gnoma Chart, a collection of one or more https://helm.sh/docs/chart_template_guide/values_files/[Helm values.yaml files] defining a deployment will be referred to as a **__deployment definition__**.  These are the only files required to be written by the end user for deploying to Kubernetes when used in conjunction with Helm and Gnoma Chart.  It was a fundamental requirement when designing Gnoma Chart that it remain 100% compatible with Helm, and only requiring values.yaml files for use was how the requirement was met.

A deployment definition consists of Gnoma Chart templates for defining what is rendered through Helm, Gnoma variables for defining any reusable data the templates need, and Gnoma deployment profiles to support different configurations among the same templates.  Gnoma Chart adds built-in Kubernetes resource definitions with reasonable default values in order to further reduce boilerplate.

=== Basic Structure

The basic structure of an Gnoma Chart deployment definition in a values.yaml file is as follows:
...
gnomaDefs-<profile or object name>__: # <1>
  ...

gnomaDefs-__<object name>__-__<profile>__: # <1>
  ...

gnomaDefs-__<profile>__-__<object name>__: # <1>
  ...

gnomaTemplates: # <2>
  ...

gnomaTemplates-__<unique identifier>__: # <2>
  ...
----
<1> `gnomaDefs(-*)` maps define  Gnoma Chart variables.
<2> `gnomaTemplates(-*)` lists define Gnoma Chart templates.

=== Deployment Profiles

**__Deployment profiles__** are the primary mechanism by which Gnoma Chart supports configuration management within a deployment definition.

Deployment profiles are typically defined dynamically on the command line in a list using the `gnomaProfiles` identifier:

`helm upgrade --install --set gnomaProfiles='{<PROFILE_1>,...,<PROFILE_N>}' ...`
...
A profile is said to be an **__active profile__** during the rendering of a deployment definition if included in the `gnomaProfiles` list.
...
Gnoma Chart templates are defined in one or more lists starting with the prefix `gnomaTemplates`:

[source,YAML,linenums,subs=+quotes]
----
gnomaTemplates-__<unique identifier>__: # <1>
...
- template: # <3>
  ...
  ...
----
<1> List of Gnoma Chart templates. The unique identifier suffix is optional.
<2> Gnoma Chart template using a built-in template.
<3> Gnoma Chart template defined by its full YAML definition.

In order to support multiple values.yaml files for flexibility and modularity when rendering deployment definitions with Helm, multiple `gnomaTemplates` lists may be defined using the optional unique identifier suffixes.  The order the lists and templates is irrelevant.  All `gnomaTemplates` lists will be concatenated before processing.  Each list name should be unique per deployment definitions, or the Helm rules for merging values.yaml will overwrite lists with matching names.  The text after `gnomaTemplates-` can be any valid YAML string.

.Example deployment definition with three `gnomaTemplates` lists
[source,YAML,linenums]
----
gnomaTemplates:
...
gnomaTemplates-my-secondList:
- templateName: <built-in-template-name>
  ...

gnomaTemplates-WITH_YA%ML:
- template:
    <full-YAML-definition>
  ...
----
...
Gnoma Chart supports two types of templates:

* **Built-in templates**: Predefined templates within Gnoma Chart.
...
All Gnoma templates, whether free-form or built-in, have a number of helper attributes.
...
- objName: obj-4
  ...
  ...
----
Given `gnomaProfiles='{PROFILE_1,PROFILE_3}'`; i.e. `PROFILE_1` and `PROFILE_3` are active:
...
**__Matrices__** are a special kind of helper attribute.  Matrices are lists of strings, and Gnoma Chart before final rendering __it will be replaced by a copy of the template__ for each element in the matrix.

Gnoma Chart currently only supports two matrix keys:
...
Gnoma Chart defines a number of pre-defined templates in order to further reduce excess boilerplate YAML, and these are referred to as **__built-in templates__**.  By convention, the names of built-in templates reflect the Kubernetes object they are to render; e.g. `deployment` for a Deployment and `horizontalPodAutoscaler` for a HorizontalPodAutoscaler.  Built-in templates are requested via the `templateName` key.
...
NOTE: The current set of built-in templates focus almost exclusively on application deployments and supporting Kubernetes resources.  It is hoped that Gnoma Chart will be able to fully support all Kubernetes resources in the future, as well as some of the more widely used Custom Resource Definitions.

===== Compound Built-in Templates

In order to further reduce excess boilerplate YAML, Gnoma Chart allows defining **__compound built-in templates__**.  Compound built-in templates combine more than one built-in template definitions into a single definition with each individual built-in re-using any shared helper attributes.  Compound built-in templates are defined as a list of one more strings under the `templateNames` key.
...
Built-in templates are a convenience, and not a necessity.  The advantage of free-form templates are that they can still use all other features of Gnoma Chart (e.g. <<_matrices, matrices>>), which means easier templating and configuration management.  Anywhere a built-in template is used a free-form template can be substituted, and vice versa if a built-in template exists for the object being rendered; however, if a `templateName` or `templateNames` helper attribute are defined, `template` will be ignored.
...
Variables are defined in YAML maps named starting with `gnomaDefs` either at the root of a document or within an Gnoma Chart <<_templates, template>> definition.  Variables may contain any any valid YAML syntax and type.

.Example Gnoma variable definitions by type
[source,YAML,linenums]
----
gnomaDefs: # <1>
  STRING: string  # <2>
  ...
  - bar
----
<1> The `gnomaDefs` map defines the default set of variables for a deployment definition.
...
TIP: Invalid variable definitions that are valid YAML are simply ignored.  It is not Gnoma Chart's place to flag valid YAML, and it should be pretty easy to debug any issues by looking at the eventual output.
...
* Variables and their references are valid YAML both as keys in maps and as values in strings, maps, and lists.  Helm requires that values.yaml files (and therefore Gnoma Chart deployment definitions) be valid YAML.

Because of the way Helm works, `gnomaDefs` variable maps are read in completely with the rest of the deployment definition, and each final variable reference value is only determined during processing; therefore, variables do **NOT** have to be defined before being referenced.
...
Variable values may be imported from one or more files. There following three prefixes tell Gnoma Chart to import data from external files:
...
Gnoma Chart defines a number of built-in variables for use in templates, Helm and template.

===== Helm Built-In Variables

Gnoma Chart has a few built-in variables derived from https://helm.sh/docs/chart_template_guide/builtin_objects/[Helm's built-in objects].

.Gnoma Chart Helm built-in variable examples
[source,YAML,linenums]
----
gnomaDefs:
...
.Gnoma Chart template built-in variable examples
[source,YAML,linenums]
----
gnomaDefs:
...
`gnomaDefs` variables have two different scopes:

* **Deployment**
+
All `gnomaDefs` variables defined outside of any Gnoma Chart templates; i.e. defined under `gnomaDefs` maps at the root of the deployment definition.  Every Gnoma template in the deployment definition may reference these variables.

* **Template**
+
All `gnomaDefs` variables defined under a specific Gnoma Chart template.  Only the specific Gnoma template may use these variables.

.Example of deployment and template `gnomaDefs` map definitions
[source,YAML,linenums]
----
gnomaDefs: # <1>
  VAR: outer
...
- templateName: <template name>
  objName: $<VAR>$<INNER_VAR>
----
<1> `gnomaDefs` defined at the root of the deployment definition are said to have **__deployment scope__**.  `VAR` can be used by `first-template` and `second-template`.
<2> `INNER_VAR` is only available to the `first-template`, because its `gnomaDefs` map is defined directly under it; therefore, the first template will be named `outerinner`, and the second `outer`.

==== Overriding

Variables may be defined or overridden in Gnoma Chart by defining more specific `gnomaDefs` maps.  The types of `gnomaDefs` maps are:

* **Profile**
+
`gnomaDefs-<PROFILE_NAME>` maps defined for a specific deployment profile.  Only one profile may be named.
* **objName**
+
`gnomaDefs-<OBJ_NAME>` maps defined for a specific `objName` or `BASE_OBJ_NAME`.  Only one `objName` may be named.
* **Profile** and **objName**
+
`gnomaDefs-<PROFILE_NAME>-<OBJ_NAME>` or `gnomaDefs-<OBJ_NAME>-<PROFILE_NAME>` maps defined for a specific deployment profile and `objName`.  Only one `objName` and/or profile may be used.

.Example of different `gnomaDefs` map definitions
[source,YAML,linenums]
----
gnomaDefs: # <1>
  VAR: a-var
...
gnomaDefs-obj-name: # <3>
  VAR: a-var

gnomaDefs-PROFILE-obj-name: # <4>
  VAR: a-var

gnomaDefs-obj-name-PROFILE: # <4>
  VAR: a-var
----
...
Each of these types of `gnomaDefs` maps may be defined at deployment or **__template scope__**.

===== Order Of Precedence

Order of precedence determines as which `gnomaDefs` map determines the ultimate value of a variable if it is defined in multiple variable maps.
...
.. Deployment profiles' precedence is from least to greatest in the `gnomaProfiles` list.
. `**gnomaDefs-<BASE_OBJ_NAME>`
+
An element from an `objNames` list.
. `**gnomaDefs-<objName>**`
+
The `objName` value of a template.
. `**gnomaDefs-<PROFILE>-<BASE_OBJ_NAME>**`
. `**gnomaDefs-<BASE_OBJ_NAME>-<PROFILE>**`
. `**gnomaDefs-<PROFILE>-<objName>**`
. `**gnomaDefs-<objName>-<PROFILE>**`

All template specific `gnomaDefs` will have precedence over deployment `gnomaDefs`.

.Example of precedence with `gnomaDefs` maps and active deployment profile `PROFILE`
[source,YAML,linenums]
----
gnomaProfiles: [PROFILE]
...
----
<1> `VAR == final-value`, because the template `gnomaDefs` overrides all deployment `gnomaDefs` maps.
<2> `VAR == an-obj-name-value`, because `gnomaDefs-<objName>` has precedence over `gnomaDefs-<profile>`.
<3> `VAR == a-profile-value`, because `gnomaDefs-<profile>` has precedence over `gnomaDefs`, and there's no matching `gnomaDefs-<objName>` map.

TIP: To null a variable out, define it as an empty value in the appropriate map.

== Rendering Process Overview

A high level overview of how Gnoma Chart processes and renders deployment definitions.  How and when variables are realized is also explained.

. **Realize Dynamic `gnomaDefs-*` Names**
+
`gnomaDefs` map names may be defined with variables; e.g. `gnomaDefs-$<FOO>`.  All of the deployment `gnomaDefs` map names are processed first.
+
IMPORTANT: **ONLY values defined in the default deployment `gnomaDefs` can be used to define deployment `gnomaDefs-*` maps.**

. **Create Profile-based `gnomaDefs`**
+
Collect final values `gnomaDefs` based on deployment profiles only; i.e. consider only `gnomaDefs-<PROFILE>` maps.

. **Collect and Filter All Templates**
+
`gnomaTemplate-*` lists are collected and concatenated to create an intermediate `gnomaTemplates` list.  Templates are then removed if their <<_discriminator_lists, discriminators>> fail.

. **Expand Matrixes:**
+
<<_matrices, Matrix>> values and lists can be parameterized with variables; e.g. `namespaces: $<NAMESPACE_LIST>` or `objNames: [$<FOO>,$<BAR>]`.  Using the profiles-based `gnomaDefs` map, process the matrix variable references, and then generate copies for any templates with matrices defined to create a final `gnomaTemplates` list.  The original template with the matrix defined is discarded.
+
IMPORTANT: **Only values derived from profile-based deployment `gnomaDefs` map can be used as variables in matrices.**

. **Process Templates**
+
For each template in the final template list:
+
.. Use the active profiles and the `objName` to derive the final deployment `gnomaDefs` for the template.
.. Using the final deployment `gnomaDefs` map as the starting point, process all template `gnomaDefs` to derive the final `gnomaDefs` map for the template.
.. Using the final `gnomaDefs` map for the template, replace ALL remaining variable references in the template.
.. If any escaped Gnoma Chart variable references exist, remove the backslash; e.g. `\$<FOO>` becomes `$<FOO>`.
...
This concludes the Gnoma Chart rendering process.

== Specialized Utilities

Gnoma Chart has a some extra functionality built in to make creating deployment definitions easier, as well as enabling new means of defining deployment definitions for more dynamic deployments.
...
In order to enable more dynamic behavior, Gnoma Chart implemented functionality that would mount a collection of ConfigMaps and/or Secrets into a projected volume by their labels.  Lists of labels can be provided, and the namespace of the Pod will be scanned for matching resources, all of which will be mounted into the container.  The values of the labels are not relevant to this functionality.  Only whether the label exists on the resource.
...
.Partial pseudo-manifest generated by Gnoma Chart from deployment definition above:
[source,YAML,linenums]
----
apiVersion: batch/v1
kind: Job
...
Gnoma Chart has implemented a utility built-in template that will copy a resource from one namespace to any other.
...
Gnoma Chart supports two utilities to help with debugging deployment definitions.

renderPreprocessedValues::

If true, output all merged values.yaml files as YAML and exit.  No Gnoma Chart processing takes place.  Templates will **NOT** be rendered to YAML.  Useful for inspecting how Helm merges multiple deployment definition files, or creating a single Gnoma Chart values.yaml file from many values.yaml files.

`helm template --set renderPreprocessedValues=true ...`

renderProcessedValues::

If true, output all values of a processed Gnoma Chart deployment definition as YAML and exit.  Includes all values belonging `gnomaTemplates` and `gnomaDefs` maps and the `gnomaProfiles` list.  Templates will **NOT** be rendered to YAML.  Useful for inspecting the results of a processed deployment definition before the templates are rendered.
+
`helm template --set renderProcessedValues=true ...`

== Unconventional Use-Cases

Below are a just a few unconventional use cases where Gnoma Chart can be used.

=== Third-party Helm Charts

There are many applications that already have Helm charts created for them, and rewriting them to use Gnoma Chart directly is not an effective use of time and resources.  So how can Gnoma Chart still be leveraged for configuration management?  The answer is to define a deployment definition that results in a values.yaml file compatible with the third-party chart, and pipe it or post-render it with a second Helm call using the target chart.
...
- kubeObject: false # <1>
  template:
    ...
    ...
----
<1> Set `kubeObject` to false so Gnoma does not automatically generate `apiVersion`, `kind`, and `metadata` fields.
...
helm template -f third-party-chart-values.yaml third-party-values-yaml gnoma-charts/gnoma-chart > final-values.yaml
...
Helm is not good at some things that Gnoma Chart cannot fix on its own; e.g. labeling and/or annotating an arbitrary collection of Kubernetes resources, oOr arbitrarily patching resources.  https://kustomize.io/[Kustomize] fills the gap in functionality that Helm in general or Gnoma Chart in particular can't address.

`kustomization.yaml` files, though, are notoriously static in nature by design.  Using Gnoma Chart to create a template of a Kustomization in order to make the files dynamic (e.g. for a CICD system) is trivial.
...
- templateName: kustomization # <2>
  fields:  # <3>
    ...
    commonLabels:
      gnoma.io/teamid: $<TEAM_ID>
      gnoma.io/projectid: $<PROJECT_ID>
----
<1> `gnomaDefs` is only defined here to note that all variables in this example are expected to be passed in via the command line.
<2> Gnoma Chart has a `kustomization` built-in template so that the boilerplate headers don't need to be defined.
...
helm template -f kustomization-values.yaml \
  --set-string gnomaDefs.RESOURCES_FILE=manifests.yaml \
  --set-string gnomaDefs.TEAM_ID=my-team \
  --set-string gnomaDefs.PROJECT_ID=my-team-project \
  kustomization-release \
  gnoma-charts/gnoma-chart > kustomization.yaml

kustomize build .
----

.Example using Gnoma Chart with a Helm `--post-renderer` to deploy third-party chart
----
helm upgrade --install -f deployment-def.yaml --post-renderer kustomize.sh my-app gnoma-charts/gnoma-chart
----

More information on using a Helm `--post-renderer` can be https://helm.sh/docs/topics/advanced/[found here].

== Extending Gnoma Chart

== Gnoma Chart Reference

=== Glossary

Terms used to help define certain concepts in Gnoma Chart.

Active profile::
Any values defined in the `gnomaProfiles` list during rendering of an Gnoma Chart.

Built-in template::
Pre-defined Gnoma Chart Templates to help make creating deployment definitions.  Referenced using the `templateName` helper attribute on an Gnoma template definition.

Compound built-in template::
An Gnoma Chart template that uses multiple built in Gnoma Chart built-in templates.  Referenced using the `templateNames` helper attribute on an Gnoma template definition.  All values in the Gnoma Chart template are shared among the listed built-templates when rendering.


Deployment definition::
A collection of Helm values.yaml files for use specifically with an Gnoma Chart.

Deployment profiles::
List of strings values matching `[A-Z]+(?:_[A-Z]+)*` assigned to `gnomaProfiles` the for use in a deployment definition. Profiles can be used as a rendering discriminator for Gnoma templates, or as an alternative set of variable definitions in an gnomaDefs-* map.

Deployment scope::
Where an Gnoma Chart variable defined.  Can be either deployment scope, in which the variable is accessible to all templates, or template, in which the variable is accessible only to the specific Gnoma Chart template where it was defined.  Template scope variables always take precedence over deployment scope variables.

Discriminator::
Any deployment profile listed in under one of the following Gnoma Chart Template properties:
...
+
Matches of active profiles with the above lists determine whether or not an Gnoma Chart Template is rendered or not.

Gnoma Chart Variables::
Keys defined under gnomaDefs maps in a deployment definition holding arbitrary YAML values.  Can be used to defined and/or reuse values in Gnoma templates.

Gnoma Chart Templates::
Member of a list under `gnomaTemplates` in a deployment definition.  Must contain either a `templateName` referencing a built-in template or a `template` attribute defining arbitrary YAML.

free-form template::
An Gnoma Chart template that does not use an Gnoma Chart template, but rather defined itself under the `template` helper attribute as arbitrary YAML.

Helper attribute::
Any attributes defined at the root of an Gnoma Chart template, which may or may not directly correlate to an attribute on the rendered resource.

Matrices::
Optional list of valid Kubernetes namespace (`namespaces`) or object resource (`objNames`) values defined per Gnoma Chart Template.  Each member of either list will result in a copy of the original template created and rendered for the particular namespace and/or name, and the original template ignored.

=== Keywords

Helm Chart `Values` settings defined for use in Gnoma Chart to help define deployment definitions.

gnomaDefaults::
Used for overriding Gnoma Chart <<_built_in_defaults, pre-defined defaults>>.

gnomaDefs::
Identifier of map for defining Gnoma Chart variables.

gnomaProfiles::
Identifier of list of strings for defining Gnoma Chart active profiles.

gnomaTemplates::
Identifier of map for defining Gnoma Chart templates.

renderPreprocessedValues::
Render the combined values.yaml files before Gnoma Chart processing.

renderProcessedValues::
Render the combined values.yaml files after Gnoma Chart processing; i.e the values.yaml file after all variables and other Gnoma Chart settings in the deployment definition have been processed, but before the chart has been rendered.
...
Built-in Gnoma Chart variables that can be used in defining other Gnoma Chart variables or within Gnoma Chart templates.
...
Default values used by built in Gnoma Chart built-in templates if not otherwise defined.
...
Default helper attributes all Gnoma templates, built-in or free-form, share.

annotations::
Kubernetes object annotations.  Maps directly to `metadata.annotations`.

gnomaDefs-*::
Gnoma variable maps.  Defines maps specific to an Gnoma template, and optional to a profile or `objName`.

kubeObject::
Unless otherwise defined for a particular Gnoma template, default value is true.  If false, the Kubernetes apiVersion and kind attributes, and the Kubernetes metadata section, Gnoma Chart will not attempt to render the metadata section.  In a free-form template, any of the above is defined, Gnoma will render it as-is without modification.  In practice, this also means the `annotations` and `labels` helper attributes are ignored.

labels::
Kubernetes object labels.  Maps directly to `metadata.labels`.

objName::
Fundamental identifier for an Gnoma template. Can be used in the following way:
...
+
`objName` is mapped to the Gnoma variable `OBJ_NAME` during rendering for each template.  It is also mapped to the Gnoma variable `BASE_OBJ_NAME` if `objNames` is not defined.

objNames::
List of string that represents a matrix value for Gnoma template `objName` identifier. Each member of the list will result in a to a copy of the template, and that member is assigned to the `objName` helper attribute.

namespace::
Namespace a rendered Gnoma template should be deployed to. Can be used in the following way:
...
+
`namespace` is mapped to the Gnoma variable `NAMESPACE` during rendering for each template.  It is also mapped to the Gnoma variable `BASE_NAMESPACE` if `namespaces` is not defined.

namespaces::
List of string that represents a matrix value for Gnoma template `namespace`. Each member of the list will result in a to a copy of the template, and that member is assigned to the `namespace` helper attribute.

template::
Arbitrary YAML to render defining the template. Ignored if `templateName` is defined.

templateName::
Name of Gnoma template to apply values to.

templateNames::
List of name of Gnoma templates to apply values to.  Each member of the list will result in a separate rendering based on the template values.
...
The following is a comprehensive list of all Gnoma Chart built-in templates and their helper attributes. While the full name of the template is given (e.g. `gnoma-kubernetes.cronJob`), only the latter part of the name is needed for rendering built-in templates (i.e. `cronjob`).
...
==== Gnoma Chart Resources

===== gnoma-renderer.copyResource
copyResource is a special Gnoma Chart helper template for copying resources already deployed to a Kubernetes cluster from one namespace to another. The objName should reflect the name of the resource to be copied in the `fromNamespace`.
...
DEFAULT: N/A +
`gnomaDefaults.ingressHostDomain`
...
|Ignored if `rules` is defined +
DEFAULT: `Prefix` +
`gnomaDefaults.ingressRulePathType`
...
|`name` is hard-coded +
DEFAULT: `TCP` +
`gnomaDefaults.prometheusProtocol`

|protocol
|spec.ports[0].protocol
|Ignored if `ports` is also defined +
DEFAULT: `TCP` +
`gnomaDefaults.protocol`

|selector
|spec.selector
|`gnoma.io/selector` will be defined in addition to any optional values here
...
===== gnoma-kubernetes.kustomization
...
IMPORTANT: If a Kubernetes resource contains a PodTemplate, the associated Gnoma template may use all podTemplate and container helper attributes in the root of the template definition.

===== gnoma-kubernetes.container
**RENDERS:** https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#Container[Container] +
**RENDERED TO:** `[<<_gnoma_kubernetes_podtemplate, PodTemplate>>].containers[0]`

Containers are rendered in various lists contained in a PodTemplate. If a <<_kubernetes_workload_resources,workload resource>> contains a PodTemplate, then these helper attributes may be expressed under the root Gnoma template for the first, main container. Other containers in either the main Pod list or other container lists must can otherwise use these helper attributes to define themselves, but should **not** use the default values.
...
|image
|image
|DEFAULT: `gnomaDefaults.image`

|imagePullPolicy
|imagePullPolicy
|DEFAULT: `Always` +
`gnomaDefaults.imagePullPolicy`
...
<<_kubernetes_workload_resources,Kubernetes workload resources>> that generate Pods all include a PodTemplate. The following helper attributes may all be defined under any of those Gnoma templates.
...
|containers
|containers
|`containers[0]` may be defined at the root of template; otherwise, see <<_gnoma_kubernetes_container, gnoma-kubernetes.container>> for helper attributes for each container in the list.
...
|ephemeralContainers
|ephemeralContainers
|See <<_gnoma_kubernetes_container, gnoma-kubernetes.container>> for helper attributes for each container in the list
...
|imagePullSecret
|imagePullSecrets
|DEFAULT: `gnomaDefaults.imagePullSecrets` +
...
|priority
|DEFAULT: `TCP`
`gnomaDefaults.imagePullSecrets`
...
Job related workload resources, <<_gnoma_kubernetes_cronjob, gnoma-kubernetes.cronJob>> and <<_gnoma_kubernetes_job, gnoma-kubernetes.job>>, contain JobSpecs. JobSpecs wrap PodTemplates, and the following helper attributes may be used on the root of the Gnoma templates.
...
|<podTemplate>
|template.spec
|See <<_gnoma_kubernetes_podtemplate, gnoma-kubernetes.podTemplate>> for further helper attributes
...
Except for the horizontalPodAutoscaler, the following Gnoma templates all contains a PodTemplate, and therefore[cols="1,2,4"] may make use of the helper attributes defined up in both <<_gnoma_kubernetes_podtemplate, gnoma-kubernetes.podTemplate>> and <<_gnoma_kubernetes_container, gnoma-kubernetes.container>>.

NOTE: Templates with PodTemplates were designed to integrate easily with both the <<_gnoma_kubernetes_service, gnoma-kubernetes.service>> and <<_gnoma_kubernetes_ingress, gnoma-kubernetes.ingress>> Gnoma templates to create <<_compound_built_in_templates, compound templates>>.
...
|<jobSpec>
|template.spec
|See <<_gnoma_kubernetes_jobspec, gnoma-kubernetes.jobSpec>> for further helper attributes
