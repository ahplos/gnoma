...
****
* Defining and deploying with arbitrary Gnoma Chart deployment profiles.
* `gnomaDefs-*` variable map definitions.
* Using Gnoma Chart matrices to further reduce copying and pasting boilerplate Kubernetes resource definitions.
****

In <<User-defined Variables>>, we demonstrated how to define and reference your own variables. In this part of the tutorial you'll learn about deployment profiles.  While the previous parts of the tutorial have focused on Gnoma Chart's strengths with regards to templating, readability, and conciseness, it's the deployment profiles that give Gnoma Chart its true power.  This feature makes it much, much easier for users to manage the same, basic deployment definitions across numerous environments with minimal configuration changes.
...
[discrete]
==== Create and save the following file as `gnoma-demo-4.yaml`
...
gnomaDefs-$<MULTI_LINGUAL_PROFILE>: # <.>
...
- templateName: configMap
  ...
  data:
    $<OBJ_NAME>: $<HTML_PAGE>
----
<.> Deployment profiles are defined whenever an Gnoma Chart template and/or `gnomaDefs` variable map is marked for them.
...
+
.. `gnomaDefs-$<MULTI_LINGUAL_PROFILE>` declares the `$<MULTI_LINGUAL_PROFILE>` (`MULTI_LINGUAL`) profile.
... Gnoma Chart template __discriminators__ define whether or not a template should be rendered based on the active profile(s).
...
<.> `objNames` is one of two Gnoma Chart matrices (`namespaces` being the other).
.. Matrices are lists that will tell Gnoma Chart to generate a copy of the template for each element in the list.
...
.. The Gnoma Chart template defines multiple ConfigMaps via the `objNames` matrix, but they will only be rendered if the `MULTI_LINGUAL` profiles is active.

<.> Gnoma Chart variables maps can be defined for a specific template.
.. `gnomaDefs` variable maps can be declared for specific profiles, a particular `objName`, and/or under specific Gnoma Chart templates.
... `objName` and/or profiles can be appended to `gnomaDefs` using a `-` as a delimiter; e.g.
.... `gnomaDefs-foo` for `objName` `foo`, which declares variables only for objects named "foo".
.... `gnomaDefs-BAR` for deployment profile `BAR`, which declares variables only when the `BAR` profile is active.
.... `gnomaDefs-foo-BAR` or `gnomaDefs-BAR-foo`, which declares variables only for objects named "foo" when the `BAR` profile is active.
.... Discriminators in `gnomaDefs` declarations can be dynamic; e.g. `gnomaDefs-$<FOO>`
.... If an `gnomaDefs-*` map ends in just a dash, or has two dashes within it because a variable used to define it is empty, it will be ignored.

==== Deploy the httpd server without an active deployment profile
:helm-update-command: helm upgrade --install --rollback-on-failure -n gnoma-chart-demo --create-namespace -f gnoma-demo-4.yaml gnoma-chart-demo gnoma-charts/gnoma-chart
...
Deploying without a deployment profile with the above deployment definition will look identical to the <<User-defined Variables>> demonstration in every way.  The lack of a proper active profile in this deployment means all the additions made to the deployment definition in `gnoma-demo-4.yaml` compared to `gnoma-demo-3.yaml` will be ignored.
...
.What to expect
****
* A Deployment, Service, Ingress, and ConfigMap will be deployed on the cluster using Gnoma Chart.
* An httpd Pod will be created from the Deployment and accessible in the browser from outside the cluster.
* The ConfigMap will contain an index.html web page that was generated from multiple Gnoma Chart variables.
...
:helm-update-command: helm upgrade --install --rollback-on-failure -n gnoma-chart-demo --create-namespace --set gnomaProfiles='{MULTI_LINGUAL}' -f gnoma-demo-4.yaml gnoma-chart-demo gnoma-charts/gnoma-chart
...
Gnoma Chart deployment profiles are defined in `gnomaProfiles` as a list.  The `helm update --install` command is modified to apply deployment profile `--set gnomaProfiles='{MULTI_LINGUAL}'` clause.  If more than one deployment profile is specified, the order of precedence is from least to greatest in determining the values of variables; e.g. given `gnomaProfiles={FOO,BAR}`, all conflicting variable definitions defined in the `BAR` profile would take precedence over `FOO`.
...
* A Deployment, Service, Ingress, and ConfigMap will be deployed on the cluster using Gnoma Chart.
* An httpd Pod will be created from the Deployment and accessible in the browser from outside the cluster.
* An index.html, english.html, and spanish.html web page will be generated from multiple Gnoma Chart variables.
