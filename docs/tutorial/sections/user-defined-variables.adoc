...
:helm-update-command: helm upgrade --install --rollback-on-failure -n gnoma-chart-demo --create-namespace -f gnoma-demo-3.yaml gnoma-chart-demo gnoma-charts/gnoma-chart
...
****
* User-defined Gnoma Chart variables in deployment definitions.
* Using Gnoma Chart variables as YAML templates.
* Easily mounting ConfigMaps in Pods with the `projectedVolumes` helper attribute.
****

Both <<Gnoma Chart Templates>> and <<Compound Gnoma Chart Templates>> made use of Gnoma Chart's built-in variables to help define their host domain.  This part of the tutorial will focus on user-defined variables.

A fundamental goal of Gnoma Chart is to eliminate the need for Helm templates and instead let the user define their deployment manifests as plain YAML documents.  Another goal is to remove the need for copying and pasting boilerplate YAML as much as possible when defining manifests.  User-defined Gnoma Chart variables support these goals.

[discrete]
==== Create and save the following file as `gnoma-demo-3.yaml`
...
<.> All Gnoma Chart variables are defined under `gnomaDefs` maps.
.. Keys in the `gnomaDefs` map are Gnoma Chart variables.
.. `gnomaDefs` maps can be any valid YAML; i.e. valid keys and values for YAML maps.
...
<.> `$<HELM_RELEASE_NAME>` is another built-in Gnoma Chart variable that corresponds to https://helm.sh/docs/chart_template_guide/builtin_objects/[`.Release.Name`] in a Helm template.
...
.. `$<OBJ_NAME>` is another built-in Gnoma Chart variable referring to the Gnoma Chart template's `objName`.
