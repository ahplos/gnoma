...
This tutorial will demonstrate how the Gnoma Chart can make defining Kubernetes manifests much easier for all involved while still taking advantage of Helm's package management functionality.

NOTE: Gnoma Chart embeds its templates inside Helm values.yaml file(s), and writing Helm templates are not required when using Gnoma Chart.  In order to avoid confusion with the word "template", Gnoma Chart values.yaml file(s) will be referred to as __deployment definitions__.
...
Gnoma Chart was created with the intention of leveraging all the package management goodness Helm has to offer while making defining deployments easier.  In particular, the following design goals were set:

Helm centric::

No extra scripts or special commands for standard Helm deployments or templating should be needed.  Gnoma Chart should be able to be used anywhere Helm can be used, and alongside any other Helm chart either as the main chart or a subchart.
...
Kubernetes is very extensible, as the abundance of https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/[Custom Resource Definitions,window=read-later] illustrates.  Gnoma Chart should be able to render CRD's out-of-the-box without requiring new Helm template code.

The following tutorial will demonstrate how Gnoma Chart met each of these goals.
